...
public class TaskEntityChangeableAspect {
    ...
    @Pointcut("execution(* *(.., ru.t1.school.open.project.entity.Task, ..)) && @annotation(ru.t1.school.open.project.aspect.annotation.Changeable)")
    public void changeTaskAnnotated() {
    }

    // Валидный формат Pointcut expression
    ...
    public void setUpChangeableMark(JoinPoint joinPoint) {
        ...
        Arrays.stream(joinPoint.getArgs())
                ...
                /* Как альтернатива можно было бы сделать в Changeable поле type() куда бы мы передавали тип класса
                   затем бы мы реализовали Chain of responsibility с хэндлерами для каждого типа
                   и так мы бы достигла случая, когда на аннотацию @Changeable регистрируется изменение
                   для каждого класса уникально: для String аналогично ниже, для какого-то класса заполнение поля changedAt,
